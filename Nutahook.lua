...
local NutaHook_data = {} do
    NutaHook_data.Author = "ayatohvh"
    NutaHook_data.update = "17.09.2025"
end
...
local menu = {
    ...
    user = main_group:label("\v\rAuthor • \v" .. NutaHook_data.Author),
    ...
    visuals = {
        ...
        watermark_color = main_group:color_picker('Color', 255, 105, 180, 1),
...
local killsay_pharases = {
    ...
    {"спи пидорас", "изи для легенды"},
    {'1', 'ублюдина'},
    {'умно сыграно', 'хуесосебаный'},
    {"слыш хуесос ты че юзаешь сакпенис сенс?"},
    {'обрыганный членосос', 'ливай нахуй'},
    {'ого пидор опять подох норм да так'},
    {'пидорасу вновь воткнули рыло в грязь'},
    {"хуесос иди мать се продай"},
    {"NutaHook.lua"},
    {"анонимно убил твою матуху", "пидорас ты не аноним не делай так больше", "сука тупой еблан"},
    {"слыш пидорас", "я те мать ебал", "цыган 2 зубый", "мутант сука 6 глаз", "опять пидорас забайтился на тт", "хуесос жирный"},
}
...
local death_say = {
    ...
    {'сыно шлюхи', 'я мать те резал'},
    {'фу нахуй', 'опять в бекшут'},
    {"везунчик блядский", "я ща те хату подожгу"}
}
...
local function watermark()
    ...
    if watermark_type == "Modern" then
        ...
        local username = string.upper(NutaHook_data.Author)
        ...
        local text2 = "Author - " .. username .. "   " .. animated_build
