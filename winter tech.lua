local mantia_data = _G.MantiaLoader or {}

local username = mantia_data.username and mantia_data.username or "???"
...
local lua_name, lua_color, script_build, altrue, m_out_pos, m_time, m_alpha, panel_height, panel_offset, panel_color, text_color = "winter", {r = 255, g = 255, b = 255}, "[alpha]", true, { ui.menu_position() }, globals.realtime(), 0, 25, 5, { 25, 25, 25, 200 }, { 255, 255, 255, 255 }
local
event_callback,
...
client.delay_call(0.1, function()
    client.color_log(255,255,255, "Welcome to winter system, " .. username)
end)
...
-- if username == "marolower" or username == "lbyking666" or username == "admin" then
--     script_build = "[вазязя]"
-- elseif username == "wmentol" then
--     script_build = " [beta]"
-- else
--     script_build = " [alpha]"
-- end
_ALKASH = username == "Alkaeshka"
_WMENTOL = username == "wmentol"
...
-- local lableb321 = label(fl_tab, "\rwinter ~" .. func.hex({menu_r, menu_g, menu_b}) .. script_build)
local calar = ui.new_color_picker("AA", "Fake lag", "\rwinter ~" .. func.hex({menu_r, menu_g, menu_b}) .. script_build, lua_color.r, lua_color.g, lua_color.b, 255)
...
local menu = {
    ...
    visualsTab = {
       ...
       -- penis = switch(aa_tab, " Penis watermark"),
        ...
        walk_cycle = switch(other_tab, "\a808080FF• \aDCDCDCFFAlways walking"),
    ...
    miscTab = {
        ...
        clantag_mode = combo(aa_tab, "Mode", {"winter", "winter 2"}),
    ...
    infoTab = {
        online = label(fl_tab, " "),
        label1 = label(fl_tab, " Welcome back " .. username),
        label1488 = label(fl_tab, " Session time: "),
        evaded = label(fl_tab, "Evaded miss to winter:"),
        killed = label(fl_tab, "Killed with winter:"),
...
local aaBuilder = {}
    for i = 1, #vars.aaStates do
        aaBuilder[i] = {
            ...
            enableState = switch(fl_tab, "\r Override " .. func.hex({menu_r, menu_g, menu_b}) .. vars.aaStates[i] .. func.hex({200,200,200})),
...
_G.notif=(function()
	...
	function a:register_callback()
		...
		client.set_event_callback("paint_ui",function()
			...
			local l={ui.get(b)}
			local m={math.floor(renderer.measure_text(nil,"[winter]"..j.draw)*1.03)}
			local n={renderer.measure_text(nil,"[winter]")}
...
winter.pattern = table.concat{'\x14\x14\x14\xFF', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF', '\x14\x14\x14\xFF', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF', '\x00\x00\x00\x00', '\x14\x14\x14\xFF'}

winter.active_tab = 1
winter.click = globals.realtime()
winter.tab_names = {'Rage', 'Misc'}
winter.tab_names_vis = {''}
local icons = {
...
window = function(x, y, w, h)
    ...
    renderer.texture(renderer.load_rgba(winter.pattern, 4, 4), x + 6, y + 6, w - 12, h - 12, 255, 255, 255, 255, 'r')
end
...
tabs = function(x, y, w, h, tab_names)
    ...
    for i, tab_name in ipairs(tab_names) do
        ...
        local visible = (inboundary(x + 6, tab_y, w - 12, tab_height) or winter.active_tab == i) and 180 or 110

        local svg = renderer.load_svg(icons[i], 40, 40)
        renderer.texture(svg, x + w/2 - 20, tab_y + tab_height/2 - 20, 40, 40, 255, 255, 255, visible)

        if inboundary(x + 6, tab_y, w - 12, tab_height) and client.key_state(0x01) and globals.realtime() > winter.click then
            winter.active_tab = i
            winter.click = globals.realtime() + 0.01
        end
    end
end
...
client.set_event_callback('paint_ui', function()
    ...
    if ui.get(tabPicker) == " Settings" and ui.get(mTabs) == " Miscellaneous" then
        ...
        tabs(sidebar_x, sidebar_y, sidebar_width, 75--[[math.floor(75 * dpi_scale)]], winter.tab_names)
    end
end)
...
event_callback("paint_ui", function()
    ...
    if data.stats.killed < 1000 then
        ui.set(menu.infoTab.killed, " Killed with winter: " .. data.stats.killed)
    else
        ui.set(menu.infoTab.killed, " Killed with winter: " .. string.sub(tostring(data.stats.killed), 0, 1) .. "." .. string.sub(tostring(data.stats.killed), 1, 1) .. "k")
    end

    if data.stats.evaded < 1000 then
        ui.set(menu.infoTab.evaded, " Evaded misses to winter: " .. data.stats.evaded)
    else
        ui.set(menu.infoTab.evaded, " Evaded misses to winter: " .. string.sub(tostring(data.stats.evaded), 0, 1) .. "." .. string.sub(tostring(data.stats.evaded), 1, 1) .. "k")
    end
    ...
    for i, feature in pairs(menu.miscTab) do
        if type(feature) ~= "table" then
            ui.set_visible(feature, isMiscTab and isEnabled and winter.active_tab == 2)
        end
	end

    for i, feature in pairs(menu.rage_tab) do
        if type(feature) ~= "table" then
            ui.set_visible(feature, isMiscTab and isEnabled and winter.active_tab == 1)
        end
	end
    ...
    ui.set_visible(menu.miscTab.clantag_mode, ui.get(menu.miscTab.clanTag) and (isMiscTab and isEnabled) and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.console_logs_custom_vibor, ui.get(menu.miscTab.console_logs) and (isMiscTab and isEnabled) and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.console_logs_resolver, ui.get(menu.miscTab.console_logs) and ui.get(menu.miscTab.console_logs_custom_vibor) and (isMiscTab and isEnabled) and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.console_logs_custom, ui.get(menu.miscTab.console_logs) and ui.get(menu.miscTab.console_logs_custom_vibor) and ui.get(menu.miscTab.console_logs_resolver) == "custom" and (isMiscTab and isEnabled) and winter.active_tab == 2)
    ...
    ui.set_visible(menu.rage_tab.auto_tp_indicator_disable, ui.get(menu.rage_tab.auto_tp) and isMiscTab and winter.active_tab == 1)
    -- ui.set_enabled(menu.rage_tab.auto_tp, false)
    ui.set_visible(menu.rage_tab.rs_mode, ui.get(menu.rage_tab.resolver) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.rs_add, ui.get(menu.rage_tab.resolver) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.rs_debug, ui.get(menu.rage_tab.resolver) and ui.get(menu.rage_tab.rs_mode) == "Random brute" and isMiscTab and winter.active_tab == 1)
    ...
    ui.set_visible(menu.rage_tab.charge_wpn, ui.get(menu.rage_tab.charge_dt) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.charge_bit, ui.get(menu.rage_tab.charge_dt) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.miscTab.ai_peek_info, ui.get(menu.miscTab.ai_peek) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.ai_peek_info_2, ui.get(menu.miscTab.ai_peek) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.drop_multi, ui.get(menu.miscTab.drop_gr) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.drop_key, ui.get(menu.miscTab.drop_gr) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.rage_tab.air_trigger, ui.get(menu.rage_tab.air_stop) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.air_stop_k, ui.get(menu.rage_tab.air_stop) and ui.get(menu.rage_tab.air_trigger) == "On hotkey" and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.air_target, ui.get(menu.rage_tab.air_stop) and ui.get(menu.rage_tab.air_trigger) == "Target priority" and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.air_distance, ui.get(menu.rage_tab.air_stop) and ui.get(menu.rage_tab.air_trigger) == "Target priority" and ui.get(menu.rage_tab.air_target) == "Target is closely" and isMiscTab and winter.active_tab == 1)

    ui.set_visible(menu.miscTab.auto_w, ui.get(menu.miscTab.auto_buy) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.auto_p, ui.get(menu.miscTab.auto_buy) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.auto_g, ui.get(menu.miscTab.auto_buy) and isMiscTab and winter.active_tab == 2)
    ui.set_visible(menu.miscTab.auto_add, ui.get(menu.miscTab.auto_buy) and isMiscTab and winter.active_tab == 2)
    ...
    ui.set_visible(menu.rage_tab.aim_tab, ui.get(menu.rage_tab.aim_tools_enable) and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_baim_mode, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 1 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_baim_trigers, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 1 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_baim_hp, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 1 and func.includes(ui.get(menu.rage_tab.aim_tools_baim_trigers), "Enemy HP < X") and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_safe_trigers, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 1 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_safe_hp, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 1 and func.includes(ui.get(menu.rage_tab.aim_tools_safe_trigers), "Enemy HP < X") and isMiscTab and winter.active_tab == 1)

    ui.set_visible(menu.rage_tab.aim_tools_hitchance_warning, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 2 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_hitchance, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 2 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_hc_land, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 2 and ui.get(menu.rage_tab.aim_tools_hitchance) and isMiscTab and winter.active_tab == 1)
    ...
    ui.set_visible(menu.rage_tab.aim_tools_silent, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 3 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_silent_out, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 3 and ui.get(menu.rage_tab.aim_tools_silent) and isMiscTab and winter.active_tab == 1)

    ui.set_visible(menu.rage_tab.aim_tools_delay_shot, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tab) == 4 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_delay_states, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tools_delay_shot) and ui.get(menu.rage_tab.aim_tab) == 4 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_delay_land, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tools_delay_shot) and ui.get(menu.rage_tab.aim_tab) == 4 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_delay_hp, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tools_delay_shot) and ui.get(menu.rage_tab.aim_tab) == 4 and isMiscTab and winter.active_tab == 1)
    ui.set_visible(menu.rage_tab.aim_tools_delay_ind, ui.get(menu.rage_tab.aim_tools_enable) and ui.get(menu.rage_tab.aim_tools_delay_shot) and ui.get(menu.rage_tab.aim_tab) == 4 and isMiscTab and winter.active_tab == 1)

    --ui.set(lableb321, text_fade_animation(3, menu_r,menu_g,menu_b, 255, "winter ~ " .. script_build))
...
ui.set_callback(menu.configTab.export, function ()
    ...
    for _, main in pairs(winter.config_data.cfg_data.anti_aim) do
        if ui.get(main) ~= nil then
            table.insert(Code[1], tostring(ui.get(main)))
        end
    end

    for _, main in pairs(winter.config_data.cfg_data.aa_other) do
        if ui.get(main) ~= nil then
            table.insert(Code[2], tostring(ui.get(main)))
        end
    end

    for _, main in pairs(winter.config_data.cfg_data.visuals) do
        if ui.get(main) ~= nil then
            table.insert(Code[3], tostring(ui.get(main)))
        end
    end

    for _, main in pairs(winter.config_data.cfg_data.misc) do
        if ui.get(main) ~= nil then
...
function saveConfig(menu, name)
    ...
    for category, data in pairs(winter.config_data.cfg_data) do
        config[category] = {}
...
function loadSettings(config)
    for category, data in pairs(config) do
        for key, value in pairs(data) do
            local element = winter.config_data.cfg_data[category][key]
            if element then
...
ui.set_callback(menu.configTab.import, function ()
    local protected = function() 
        for k, v in pairs(json.parse(base64.decode(clipboard.get()))) do
            ...
            if not winter.config_data.cfg_data[k] then
                error("category not found: " .. tostring(k))
                return
            end

            for k2, v2 in pairs(v) do
                if not winter.config_data.cfg_data[k][k2] then
                    error("element in config not found: " .. tostring(k) .. "[" .. tostring(k2) .. "]")
                    return
                end

                if v2 == "true" then
                    ui.set(winter.config_data.cfg_data[k][k2], true)
                elseif v2 == "false" then
                    ui.set(winter.config_data.cfg_data[k][k2], false)
                else
                    ui.set(winter.config_data.cfg_data[k][k2], v2)
                end
            end
        end
    end
...
local tt = {
    default_kill = {
        'Думал, снежком кинуть и победить? Вот тут ты промахнулся!',
        'Твои навыки на льду? Вот тут даже сугроб не впечатлился!',
        'Смотри, как я на санках, а ты в сугробе застрял, вот тут!',
        'Хотел каток зажечь? Вот тут ты только лёд поцарапал!',
        'Твоя зимняя куртка не спасёт от моего вайба, вот тут!',
        'Лови снежок, слабак, вот тут тебе не жарко!',
        'Думал, на лыжах улетишь? Вот тут ты в сугробе утонешь!',
        'Твой стиль катания? Вот тут даже пингвины ржут!',
        'Хотел снеговика крутого слепить? Вот тут комок рассыпался!',
        'Мороз тебя не щадит, а я тут жарю, вот тут!',
        'Твои варежки тёплые? Вот тут они тебе не помогут!',
        'Думал, на коньках звезда? Вот тут ты ледяной клоун!',
        'Снег под тобой тает от стыда, вот тут!',
        'Хотел в зимнем вайбе трон занять? Вот тут я король!',
        'Твоя шапка с помпоном? Вот тут она в сугробе тонет!',
        'Думал, снежную крепость построишь? Вот тут я её сношу!',
        'На лыжне ты как черепаха, вот тут я тебя обгоняю!',
        'Твой горячий шоколад остыл, вот тут я пью победу!',
        'Хотел зиму завоевать? Вот тут метель тебя уносит!',
        'Твои коньки тупые, как твой план, вот тут!',
        'Снежки кидаешь, как ребёнок? Вот тут я артиллерия!',
        'Думал, в сугробе спрятаться? Вот тут я тебя найду!',
        'Твой зимний стиль? Вот тут даже ёлка моднее!',
        'Хотел на катке понтануться? Вот тут ты падаешь смешно!',
        'Мороз твой союзник? Вот тут я его растоплю!',
        'Твоя снежная баба? Вот тут она уже лужа!',
        'Думал, на санках всех уделать? Вот тут я турбо-режим!',
        'Твой ледяной взгляд? Вот тут он просто замерзает!',
        'Хотел зимний вайб поймать? Вот тут я его украл!',
        'Твои валенки в снегу утонули, вот тут я чемпион!',
    },
    rus_kill = {
        'Ой, убили? Вот тут я всё равно в сугробе круче лежу!',
        'Попал в меня? Вот тут мой снежок уже летит в ответ!',
        'Снёс меня? Вот тут я в метели вернусь с реваншем!',
        'Уложил в снег? Вот тут я всё равно каток зажгу!',
        'Добил на льду? Вот тут мои коньки ещё заточатся!',
        'Сугробом завалил? Вот тут я выкопаюсь и отомщу!',
        'Скинул с санок? Вот тут я на лыжне тебя догоню!',
        'Заморозил меня? Вот тут мой горячий вайб тебя растопит!',
        'Попал снежком? Вот тут я крепость построю и вернусь!',
        'Уронил на катке? Вот тут я встану и зажгу лёд!',
        'Засыпал снегом? Вот тут я снеговиком восстану!',
        'Убил в метели? Вот тут я бурю на тебя обрушу!',
        'Снёс в сугроб? Вот тут я с лыжами вернусь, держись!',
        'Провалил мой план? Вот тут я новый в снегу придумаю!',
        'Уложил на лёд? Вот тут мой стиль всё равно ярче!',
        'Замел мои следы? Вот тут я в варежках тебе вмажу!',
        'Скинул с горы? Вот тут я на санках верну трон!',
        'Попал в меня? Вот тут мой снежный ком уже в пути!',
        'Убил на катке? Вот тут я тройной тулуп накручу!',
        'Затоптал в снегу? Вот тут я снежную бурю подниму!',
        'Снёс мою защиту? Вот тут я ледяной стеной вернусь!',
        'Заморозил мой вайб? Вот тут я оттаю и зажгу!',
        'Уронил в сугроб? Вот тут я снеговиком восстану!',
        'Погасил мой запал? Вот тут я зимний огонь разожгу!',
        'Сбил с лыжни? Вот тут я турбо-режим включу!',
        'Засыпал снегом? Вот тут я метелью тебя накрою',
        'Убил в снегу? Вот тут я ледяным взглядом отомщу!',
        'Снёс на льду? Вот тут я коньками искры вырежу!',
        'Завалил в сугроб? Вот тут я с горячем чаем вернусь!',  
        'Добил в метели? Вот тут я зимним королём восстану!',
    }
}
...
event_callback("paint", function()
    if ui.get(menu.visualsTab.cros_ind) then
        ...
        local main_text_x = X / 2 + (vector(renderer.measure_text("c-", "winter alpha")).x / 2 + 5) * fraction + scope_offset
        ...
        renderer.text(
            ...
            "c-", 
            0, 
            string.upper("winter  " .. text_fade_animation(5, r, g, b, 255 * fraction_alpha, "alpha"))
        )
    ...
    if ui.get(menu.visualsTab.wtm_enable) then
        if ui.get(menu.visualsTab.wtm_style) == "Default" then
            ...
            local watermaro4ka = animate_text(globals.curtime(), lua_name .. " ~ alpha", clr_r, clr_g, clr_b, 255)
            renderer.text(sizeX/2 - renderer.measure_text("db", lua_name .. " alpha")/2, sizeY - 20, 155, 0, 0, 0, "db", nil, unpack(watermaro4ka))
        elseif ui.get(menu.visualsTab.wtm_style) == "Glowed" then
            local col1, col2, col3, col4 = ui.get(calar)
            local sizeX, sizeY = client.screen_size()
            
            local nick = username:sub(1, 8)
        ...
        elseif ui.get(menu.visualsTab.wtm_style) == "Modified" then
            ...
            renderer.text(wmX + 10, wmY + 4, 255, 255, 255, 255, nil, 0, "winter /")
            renderer.text(wmX + 55, wmY + 4, col1, col2, col3, 255, nil, 0, text_fade_animation(3, col1, col2, col3, 255, "alpha"))

            round_rect(fpsX, fpsY, 100, 20, 8, {10, 10, 10, 150})
            blur_line(fpsX + 5, fpsY + 20, 90, 2, col1, col2, col3, 255, 5)
            renderer.text(fpsX + 5, fpsY + 4, 255, 255, 255, 255, nil, 0, username:sub(1, 8) .. " /")
        ...
        elseif ui.get(menu.visualsTab.wtm_style) == "Newest" then
            ...
            local watermark_base = "winter.lua - " .. string.lower(username) .. " / " .. string.lower(script_build)
            ...
            func.render_text(pos.x + 10/2 - 20, text_pos.y + size.y*1.5 + 20, {255, 255, 255, watermark_data.opacity, "Current version - "}, {cr, cg , cb, watermark_data.opacity, "alpha"})
...
event_callback('net_update_end', function()
    ...
    if ui.get(menu.miscTab.clanTag) then
        if ui.get(menu.miscTab.clantag_mode) == "winter" then
            if clantag ~= clantag_prev then
                clantag_prev = clantag
                client.set_clan_tag(clantag)
            end
        elseif ui.get(menu.miscTab.clantag_mode) == "winter 2" then
            client.set_clan_tag("winter")
        end
    end
...
function console_hit(e)
    ...
    color_log(color[1], color[2], color[3], "winter ~ ")
...
function console_log(e)
    ...
    color_log(highlight_color[1], highlight_color[2], highlight_color[3], "winter alpha ~ ")
...
function save_level_data()
    writefile(level_data_file, winter.level_system.level .. "," .. winter.level_system.xp)
end

winter.level_system.level, winter.level_system.xp = load_level_data()



function calculate_level_and_progress(level)
    local current_level = winter.level_system.level
...
event_callback("player_death", function(event)
    ...
    if target ~= my.entity and attacker == my.entity then
        winter.level_system.xp = winter.level_system.xp + client.random_int(5, 20)
        local _, _, threshold = calculate_level_and_progress(winter.level_system.xp)

        if winter.level_system.xp >= threshold then
            winter.level_system.level = winter.level_system.level + 1
            winter.level_system.xp = 0
        end
        save_level_data()
    end
end)
...
function debug_new()
    ...
    local text_width = renderer.measure_text(nil, "winter")
