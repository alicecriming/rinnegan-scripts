if not pcall(client.create_interface) then
	error("[Starveil.debug]: error: please make sure Allow unsafe scripts is enabled and try again")
	return
end
print("welcome to Starveil, the number 1 anti aim lua.")
...
local Starveil = {}
Starveil.__index = setmetatable(Starveil, {})
Starveil.data = {
	Flip = false,
	BodyYaw = 0,
	SelfHeight = 0,
	...
	LastUpdateTimer = "2025.08.24 -> (Time: 20.00)",
	...
	Configs = database.read("[Starveil.debug]Configs") or {},
	SelfName = panorama.open().MyPersonaAPI.GetName(),
	ConfigsList = database.read("[Starveil.debug]Configs Data") or {},
	BaseConfig = database.read("[Starveil.debug]Base Config") or false,
	IdealGradientColors = {{1, 59, 175, 255}, {202, 70, 205, 255}, {201, 227, 58, 255}},
	LocaltionLastUpdateTimer = database.read("[Starveil.debug]Fired Last Update Timer"),
	PlayerState = {"Global", "Stand", "Run", "Slow", "Air", "Air+", "Crouch", "Crouch+", "Onshot", "FL"},
	Trace = require("gamesense/trace") or error("[Starveil.debug]error: failed load library: gamesense/trace, please go to lua workshop and subscribe this library"),
	Icons = require("gamesense/icons") or error("[Starveil.debug]error: failed load library: gamesense/icons, please go to lua workshop and subscribe this library"),
	Entity = require("gamesense/entity") or error("[Starveil.debug]error: failed load library: gamesense/entity, please go to lua workshop and subscribe this library"),
	Base64 = require("gamesense/base64") or error("[Starveil.debug]error: failed load library: gamesense/base64, please go to lua workshop and subscribe this library"),
	Surface = require("gamesense/surface") or error("[Starveil.debug]error: failed load library: gamesense/surface, please go to lua workshop and subscribe this library"),
	Clipboard = require("gamesense/clipboard") or error("[Starveil.debug]error: failed load library: gamesense/clipboard, please go to lua workshop and subscribe this library"),
	CsgoWeapons = require("gamesense/csgo_weapons") or error("[Starveil.debug]error: failed load library: gamesense/csgo_weapons, please go to lua workshop and subscribe this library"),
...
Starveil.__index.ContainText = function(self, list, match)
...
Starveil.__index.ToInteger = function(self, var)
	return math.floor(var + 0.5)
end

Starveil.__index.GetTimeScale = function(self)
	return client.timestamp() / 1000
end

Starveil.__index.BindArg = function(self, handler, address)
	return function(...)
		return handler(address, ...)
	end
end

Starveil.__index.ToGradientColor = function(self, text)
	return self:RgbaToHexGradientText(self.data.IdealGradientColors[1], self.data.IdealGradientColors[2], text)
end

Starveil.__index.RgbToHexText = function(self, colors, text)
...
Starveil.__index.RgbaToHex = function(self, colors)
...
Starveil.__index.RgbaToHexText = function(self, colors, text)
...
Starveil.__index.Clamp = function(self, var, min, max)
	local math_vars = self:GetMathVars(min, max)
	return math.max(math.min(var, math_vars.max), min)
end

Starveil.__index.GetMathVars = function(self, ...)
...
Starveil.__index.Contains = function(self, tab, this)
...
Starveil.__index.Lerp = function(self, current, target, percentage)
...
Starveil.__index.HexTextToOriginal = function(self, text)
...
Starveil.__index.GetStringBytes = function(self, string, index)
	local count_byte = 1
...
Starveil.__index.UTF8DecodeSupport = function(self, string)
	local string_list = {}
	local last_index = 0
	local last_count = 1
	local start_count = 0
...
Starveil.__index.RgbaToHexGradientText = function(self, color_1, color_2, text)
	local gradient_text = ""
...
Starveil.__index.IsElement = function(self, element)
	return type(element) == "table" and element.metatype and element.metatype == "Element"
end

Starveil.__index.FirstUpper = function(self, text, length)
	return text:sub(1, length or 1):upper()
end

Starveil.__index.FirstUpperColor = function(self, text, color)
	if color then
...
Starveil.__index.NewSliderIdentification = function(self, list)
...
Starveil.__index.ContainsUI = function(self, ui, list)
...
Starveil.__index.MultiColorText = function(self, data)
	local CurrentText = ""
...
Starveil.__index.NewElement = function(self, ref, list)
...
Starveil.__index.WritePackage = function(self, group, key)
...
Starveil.__index.GetPackages = function(self, elements)
	local Configs = {}
	self.data.ConfigPackage = {}
...
Starveil.__index.FiredUpdateLog = function(self)
	if self.data.LocaltionLastUpdateTimer ~= self.data.LastUpdateTimer then
		...
		table.insert(self.data.NotifyCached, {
			...
			Title = ("Starveil%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug updated !")),
...
Starveil.__index.CopyCurrentConfigs = function(self, elements)
	...
	table.insert(self.data.NotifyCached, {
		...
		Title = ("Starveil%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug")),
...
Starveil.__index.PasteCurrentConfigs = function(self, elements)
	self.data.ConfigPackage = {}
	self:WritePackage(elements)
	local Configs = self.data.Clipboard.get()
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	if not Configs then
		client.error_log(("[%s] Config Failed Paste: The Clipboard Data Is Starveil Data"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "The Clipboard Data Is Starveil Data")),
			Title = ("Starveil%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
			}
	...
	if not Successed or not JsonData then
		client.error_log(("[%s] Config Failed Paste: The Config Is Starveil Config, Please Check Your Clipboard"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			UserName = ("User: %s"):format(self.data.SelfName),
			AnimationColor = self.data.NotifyWarningAnimationColor,
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "This Config Is Starveil Config")),
			Title = ("Starveil%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
...
Starveil.__index.CreateElements = function(self)
	self.Elements = {
		...
		AntiAim = {
			...
			OwnerLabel = self:NewElement(ui.new_label(tab, cont, "                     Owner: ayatohvh")),
...
Starveil.__index.HandleReference = function(self, state)
...
Starveil.__index.HandleMain = function(self)
...
Starveil.__index.NormalizedYaw = function(self, yaw)
	if not yaw then
		return 0
	end
...
Starveil.__index.NormalizedAngles = function(self, angles)
...
Starveil.__index.CreateSurfaceFont = function(self, name, size, width, flags)
...
Starveil.__index.RegisteredCallBack = function(self, key, handle)
...
Starveil.__index.MultiCallBack = function(self, tab, fn, call_init)
	if call_init then
		fn()
	end
...
Starveil.__index.GetIcon = function(self, name, type, color)
...
Starveil.__index.CreateAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
...
Starveil.__index.ProxyBind = (function()
...
Starveil.__index.CreateCHelpers = function(self)
...
Starveil.__index.GetCvar = function(self)
...
Starveil.__index.CreateRelativeAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
	local decode = self:UTF8DecodeSupport(text)
	if decode.length <= 0 then
...
Starveil.__index.NewEntity = function(self, ent)
	return setmetatable({
		ent = ent,
...
Starveil.__index.GetTickcountSwitch = function(self, ticks)
	return self.data.UpdateTickcountState[ticks]
end

Starveil.__index.Vector2DValid = function(self, vector, radius)
	return vector and vector.x ~= 0 and vector.y ~= 0 and not self:OffScreen(vector, radius)
end

Starveil.__index.OffScreen = function(self, position, radius)
...
Starveil.__index.UpdateUseKey = function(self, state)
...
Starveil.__index.MicroMove = function(self, cmd)
...
Starveil.__index.ExtrapolatePosition = function(self, player, origin, ticks)
...
Starveil.__index.UpdateTickcount = function(self, tickcount)
	for index = 2, 128 do
...
Starveil.__index.CalculateYaw = function(self, local_angle, enemy_angle)
...
Starveil.__index.GetLerpAnimation = function(self, key, set_var)
...
Starveil.__index.GetStaticTargetAnimation = function(self, key, set_var)
...
Starveil.__index.GetTimerSwitch = function(self, timer)
...
Starveil.__index.NewLerp = function(self, current, target, animation, key)
	if not self.data.CachedLerp[key] then
...
Starveil.__index.GetTimerSwitchWays = function(self, timers, data)
	local CachedKey = ""
...
Starveil.__index.CreateStaticResetAnimation = function(self, min, max, condition, animation, key)
	local math_vars = self:GetMathVars(min, max)
...
Starveil.__index.CreateStaticTargetAnimation = function(self, current, target, animation, key)
	if not self.data.CachedStaticSingleAnimation[key] then
...
Starveil.__index.Update = function(self, min, max, between, key)
...
Starveil.__index.Cycle = function(self, min, max, between, key)
	local between = math.abs(between)
	local is_reserved = min < 0 and max < 0
	local math_vars = self:GetMathVars(min, max)
...
Starveil.__index.CreateWays = function(self, ways, angles)
...
Starveil.__index.AutomaticSaveConfig = function(self)
...
Starveil.__index.ExportConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	self.data.Clipboard.set(self.data.Base64.encode(json.stringify(ConfigPackage)))
...
Starveil.__index.ImportConfig = function(self)
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	local Configs = self.data.Clipboard.get()
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	if not Configs then
		client.error_log(("[%s] Config Failed Import: The Clipboard Data Is Starveil Data"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
	...
	local Successed, JsonData = pcall(self.data.Base64.decode, Configs)
	if not Successed or not JsonData then
		client.error_log(("[%s] Config Failed Import: The Config Is Starveil Config, Please Check Your Clipboard"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
Starveil.__index.DeleteConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	...
	if not self.data.ConfigsList[ConfigName] then
		client.error_log(("[%s] Config Failed Delete: This Config Is Starveil"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
Starveil.__index.CreateConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
...
Starveil.__index.SaveConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	local ConfigIndex = self.Elements.Configs.ConfigsList:get()
	if not ConfigIndex then
	...
	if not ConfigName then
		client.error_log(("[%s] Config Failed Save: You Cant Save a Starveil Config"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			UserName = ("User: %s"):format(self.data.SelfName),
			AnimationColor = self.data.NotifyWarningAnimationColor,
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Save a Starveil Config")),
			Title = ("Starveil%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
...
Starveil.__index.LoadConfig = function(self, base_config)
	local Configs = {}
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	...
	if not BaseConfigs or type(BaseConfigs) ~= "string" then
		if not self.data.ConfigsList[ConfigName] then
			client.error_log(("[%s] Config Failed Load: You Cant Load a Starveil Config"):format(self.data.ScriptName))
			table.insert(self.data.NotifyCached, {
				Switch = false,
				CurrentIndex = false,
				Timer = self:GetTimeScale(),
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Starveil Config")),
				Title = ("Starveil%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
	...
	if not Successed or not JsonData then
		if type(base_config) ~= "string" then
			...
			table.insert(self.data.NotifyCached, {
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Starveil Data Config")),
				Title = ("Starveil%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".debug -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
...
Starveil.__index.GetPlayerState = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
		return
	end
...
Starveil.__index.ManualAntiAim = function(self)
...
Starveil.__index.CanAttack = function(self, player, only_fire, adder)
...
Starveil.__index.RectangleOutline = function(self, x, y, w, h, round, thickness, color, settings, round_percentage)
...
Starveil.__index.RenderRoundRectangle = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderLeftOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderRightOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderBackground = function(self, x, y, w, h, color)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderGlowText = function(self, x, y, color, flags, max_width, text, glow_radius, ref_text)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderGlowOutline = function(self, x, y, w, h, width, rounding, color, settings, round_percentage)
	if color[4] <= 0 then
		return
	end
...
Starveil.__index.RenderGlowOutlineRectangle = function(self, x, y, w, h, rounding, glow_thickness, colors, settings, round_percentage)
	if settings then
...
Starveil.__index.TraceClosestPointRay = function(self, ray_from, ray_to, desired_point)
...
Starveil.__index.UpdateHistory = function(self, position, ticks, key)
...
Starveil.__index.CalculateFreestanding = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
...
Starveil.__index.Render3DCircle = function(self, x, y, z, radius, colors, accuracy, width, outline, start_degrees, percentage, removed_radius, style)
...
Starveil.__index.ShouldAntiAim = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
		return false
	end
...
Starveil.__index.UpdateDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
...
Starveil.__index.UpdateTickbaseDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
...
Starveil.__index.NetUpdateStart = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Starveil.__index.NetUpdateEnd = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Starveil.__index.FastLadder = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
...
Starveil.__index.PlayerHurt = function(self, e)
...
Starveil.__index.BulletImpact = function(self, e)
...
Starveil.__index.AimHit = function(self, e)
...
Starveil.__index.AimMiss = function(self, e)
	if not e.target or not self.Elements.Visuals.HitlogIndication:get() or not self.Elements.MasterSwitch:get() then
		return
	end
...
Starveil.__index.PreRender = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
...
Starveil.__index.Render = function(self)
	local tickcount = globals.tickcount()
	...
	if WatermarkFraction > 0 then
		...
		local PrevWatermarkText = ("%s%s%s%s%s%s%s%s"):format(
			...
			self:RgbaToHexText(whitecolor, ("%s  "):format("win")) or "",
...
--last part
	...
	if DebugFraction > 0 then
    	...
    	--------wide height
		local NameTextSize = self.data.Vector(renderer.measure_text("Starveil.\aA8A5F1FFdebug"))
		local GlowThickness = (100 / 10) or 10
		local GlowFraction = 100 or 70
		...
		self:RgbaToHexGradientText(DebugPanelColor, DebugPanelColor, "Starveil.\aA8A5F1FFdebug")
		renderer.text(x + 45, y - 5, 220, 220, 220, 255 * DebugFraction, "c-", 0, "Starveil.\aA8A5F1FFdebug        R")
	...
	if CrosshairFraction > 0 then
		...
		local PlayerStateTextSize = self.data.Vector(renderer.measure_text(CrosshairStyle == "Manifold" and "c-" or "c-", CrosshairStyle == "Manifold" and ("Starveil / %s"):format(("%i"):format(VelocityModifier * 100), "%") or self.data.PlayerStateName:upper()))
...
Starveil.__index.TickBase = function(self, e)
	local FakeDuck = ui.get(self.References.fake_duck)
	local local_player = self:NewEntity(entity.get_local_player())
	local LagCompensation = self:GetCvar("cl_lagcompensation")
...
Starveil.__index.FakeLag = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
...
Starveil.__index.AntiAim = function(self, e)
...
Starveil.__index.PaintUI = function(self)
	self:Render()
end

Starveil.__index.SetupCommand = function(self, e)
	self:AntiAim(e)
	self:FakeLag(e)
	self:TickBase(e)
	self:FastLadder(e)
...
Starveil.__index.PostConfigLoad = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
Starveil.__index.PostConfigSave = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
	...
	if AutomaticSaveConfig then
		local ConfigPackage = self:GetPackages(self.Elements)
		database.write("[Starveil.debug]Configs", self.data.Configs)
		database.write("[Starveil.debug]Configs Data", self.data.ConfigsList)
		self.data.ConfigsList[self.data.AutoSaveConfigName] = self.data.Base64.encode(json.stringify(ConfigPackage))
	end
...
Starveil.__index.ShutDown = function(self)
	self:UpdateUseKey(true)
	...
	local local_player = self:NewEntity(entity.get_local_player())
	database.write("[Starveil.debug]Configs Data", self.data.ConfigsList)
	database.write("[Starveil.debug]Fired Last Update Timer", self.data.LastUpdateTimer)
	database.write("[Starveil.debug]Base Config", self.data.Base64.encode(json.stringify(ConfigPackage)))
	for index, material in pairs(self.data.ZoomMaterials) do
		if material ~= nil then
			material:reload()
		end
...
Starveil.__index.CallBacks = function(self)
	return {
		["aim_hit"] = function(e)
			self:AimHit(e)
		end,
...
Starveil.__index.Work = function(self)
	jit.on()
	self:CreateCHelpers()
	self:CreateElements()
	self:FiredUpdateLog()
